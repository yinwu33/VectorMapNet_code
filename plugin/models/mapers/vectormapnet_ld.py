...
class VectorMapNetLD(BaseMapper):
    ...
    def __init__(self,
        ...
        super(VectorMapNetLD, self).__init__()
    ...
    def forward_train(self, polys, points=None, img=None, img_metas=None, **kwargs):
        '''
        Args:
        ...
        batch, img_metas, valid_idx, points = self.batch_data(
            polys, img_metas, points[0].device, points)
        ...
        num_sample = len(points) # img.size(0)  # batch size

        return loss, log_vars, num_sample

    @torch.no_grad()
    def forward_test(self, polys, points=None, img=None, img_metas=None, **kwargs):
        '''
            inference pipeline
        '''
        ...
        results_list = self.head.post_process(preds_dict, None, batch, only_det=self.only_det)

        return results_list

    def batch_data(self, polys, img_metas, device, points=None):
        ...
        return batch, img_metas, valid_idx, points
